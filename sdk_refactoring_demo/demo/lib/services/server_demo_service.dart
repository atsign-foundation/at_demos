...
class ServerDemoService {
  ...
  Future<bool> put(AtKey atKey, String value) async => await getAtClientForAtsign().put(atKey, value);

  Future<bool> delete(AtKey atKey) async => await getAtClientForAtsign().delete(atKey);

  Future<List<AtKey>> getAtKeys({String? sharedBy}) async => await getAtClientForAtsign().getAtKeys(
  ...
  void startMonitoring(String atsign, Function notificationCallback) async {
    ...
    await getAtClientForAtsign().startMonitor(
        notificationCallback, (error) => _logger.severe('Starting monitor throws $error'), monitorPreference);
  }

  MonitorService getMonitorService(String atSign, Function notificationCallBack, Function errorCallBack) {
    ...
    return _monitorService =
        new MonitorService(notificationCallBack, errorCallBack, atSign, preference, monitorPreference);
  }
  ...
  Future<void> syncOnce(Function callBack, Function errorCallBack) async {
    await getAtClientForAtsign().getSyncManager().syncOnce(callBack, errorCallBack);
  }
  ...
  Future<void> notify(String value, String receiverAtsign, {Function doneCallBack, Function errorCallBack}) async {
    try {
      ...
      await getAtClientForAtsign().notify(atKey, value, OperationEnum.update, (value) {
